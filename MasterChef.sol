...
// MasterChef is the master of POLYFEED. He can make POLYFEED and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once POLYFEED is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint16 depositFeeBP;      // Deposit fee in basis points (i.e. 100bp = 1%)
    }
    ...
    // POLYFEED tokens created per block.
    uint256 public feedPerBlock = 0.05 ether;
    // Bonus muliplier for early polyfeed makers.
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(feedReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 feedReward = multiplier.mul(feedPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        feed.mint(devaddr, feedReward.div(10));
        feed.mint(address(this), feedReward);
        pool.accEggPerShare = pool.accEggPerShare.add(feedReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe feed transfer function, just in case if rounding error causes pool to not have enough EGGs.
    ...
    function updateEmissionRate(uint256 _feedPerBlock) public onlyOwner {
        massUpdatePools();
